...
/**
 * connects to remote server which exposes RPC calls and creates all
 * @param {String} url to connect to, for example http://localhost:8080
 * @param {Object} handshake for global authorization that is passed to socket.io
...
module.exports = function RPCBackend(url, handshake) {
    ...
    var callEnded = function (Id) {
        if (deferreds[Id]) {
            delete deferreds[Id];
            endCounter++;
            self.onEnd(endCounter);
            if (endCounter == invocationCounter) {
                self.onBatchEnd(endCounter);
                invocationCounter = 0;
                endCounter = 0;
            }
        }else {
        ...
        serverRunDateDeferred.promise.then(function () {
            ...
            if (cached) {
                ...
                if (serverRunDate < new Date(cached.cDate)) {
                    ...
                    masterChannel.emit('load channel', {name: name});
                }
            } else {
                masterChannel.emit('load channel', {name: name});
            }
        ...
        data.fnNames.forEach(function (fnName) {
            channel[fnName] = function () {
                ...
                if (invocationCounter == 1) {
                    self.onBatchStarts(invocationCounter);
                }
                self.onCall(invocationCounter);
     ...
     * @returns {Promise}
     */
    self.loadChannel = function (name) {
        if (serverChannels.hasOwnProperty(name)) {
     ...
     * @returns {Promise}
     */
    self.expose = function (name, toExpose) { //
