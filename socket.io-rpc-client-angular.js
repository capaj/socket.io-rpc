...
angular.module('RPC', []).factory('$rpc', function ($rootScope, $log, $q) {
     ...
     */
    function Connect(url, handshake) {
        ...
        if (!Connect.defaultBackend) {
            Connect.defaultBackend = rpc;   //the first rpc connection is the default, if you want, you can set some other
        }
        return rpc;
		...
		compile: function compile(tEl, tAttrs) {
			return {
				pre: function (scope, iElement, attr, controller) {
                    ...
                    var backend = scope.$eval(attr.rpcBackend) || $rpc.defaultBackend;
                    ...
                    if (attr.rpcAuth) {
						var authGetter = scope.$eval(attr.rpcAuth);
                        ...
                        $q.when(authGetter()).then(function (handshake) {
                            ...
                            var promise = backend.loadChannel(attr.rpcChannel, handshake);
                            instantiate(promise);
                        });
					} else {
